...
<style>
  @import url('../assets/css/rice-roll.css');
</style>
<script>
  export default {
    mounted: function(){
    ...
    methods: {
      changeFond(){
        let couleurFond;
        let numFond = Math.floor((Math.random() * 10) + 1);
        switch (numFond) {
          case 1:
            couleurFond = "#16a085";
            break;
        ...
        // this.$refs.slime_conteneur.style.backgroundColor = couleurFond;
      },
      changeCouleur(){
        let tabNum=[];
        while (tabNum.length<4){
          let numSlime = Math.floor((Math.random() * 10) + 1);
          if (tabNum.indexOf(numSlime) == -1){
            tabNum.push(numSlime);
          }
        }
        let corpsSlime = document.getElementsByClassName("corps");
        for (let i = 0; i < 4; i++) {
          let couleurSlime;
          switch (tabNum[i]) {
            case 1:
